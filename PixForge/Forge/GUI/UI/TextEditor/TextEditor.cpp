...
void PF::Forge::Ui::TextEditor::render() {
  ...
  if(ImGui::BeginMenuBar()){
    ...
    if(ImGui::Button("exit")) { open = false; Tools::Log::get().log("Text Editor UI Window Closed: " + file.getPath() );}
    ImGui::EndMenuBar();
  };
  ...
  if(ImGui::Button("save")){  
    ...
    file.push(text);
    file.save();
    Tools::Log::get().inf("File Saved: " + file.getPath());
  };
...
void PF::Forge::Ui::TextEditor::read(){
  ...
  Tools::Log::get().inf("File Read: " + file.getPath());
};